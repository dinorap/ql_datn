...
import { TypeCompanies } from "../../../components/Header&Footer/Companies";
...
const ProductsPage = () => {
    ...
    useEffect(() => {
        ...
        fetch();
    }, [currentPage, filters, sortKey, sortOrder, selectedRanges, filterAll]);
    ...
    const matchedCategory = filterAll?.category?.find(c =>
        slugify(c.label) === categoryLabel
    );
    const categoryId = matchedCategory?.value || null;
            ...
            {categoryId ? (
                <TypeCompanies
                    ...
                    onCompanySelect={(companyId) => {
                        const currentCompanyIds = (filters.company_id || "").split(',').filter(Boolean);

                        let updatedCompanyIds;
                        if (currentCompanyIds.includes(String(companyId))) {
                            updatedCompanyIds = currentCompanyIds.filter(id => id !== String(companyId));
                        } else {
                            updatedCompanyIds = [...currentCompanyIds, String(companyId)];
                        }

                        const newFilters = {
                            ...filters,
                            company_id: updatedCompanyIds.length ? updatedCompanyIds.join(',') : null,
