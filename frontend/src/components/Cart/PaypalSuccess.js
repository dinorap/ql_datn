...
import { capturePaypalOrder } from "../../services/apiPayment";
...
const PaypalSuccess = () => {
    ...
    useEffect(() => {
        ...
        if (!token) return setStatus(false);
    ...
    return (
        <div className="vnpay-success-container">
            {status === null ? (
                <h3>Đang xử lý thanh toán PayPal...</h3>
            ) : status === true ? (
                <div className="card1 success1">
                    <div className="icon"><img src='/mark.png' alt="success" /></div>
                    <h2>Thanh toán thành công</h2>
                    <p>Đơn hàng của bạn đã được thanh toán qua PayPal. Chúng tôi sẽ sớm liên hệ để giao hàng.</p>
                    <button onClick={() => navigate("/cart")}>Quay lại giỏ hàng</button>
                </div>
            ) : (
                <div className="card1 fail">
                    <div className="icon"><img src='/cancel.png' alt="fail" /></div>
                    <h2>Thanh toán thất bại</h2>
                    <p>Giao dịch PayPal không thành công. Vui lòng thử lại hoặc liên hệ hỗ trợ.</p>
                    <button onClick={() => navigate("/cart")}>Quay lại giỏ hàng</button>
                </div>
            )}
        </div>
